...
public class TimeWheel {
    ...
    public TimeWheel(long tickMs, int wheelSize, long currentTime, DelayQueue<TimerTaskList> delayQueue) {
        ...
        log.info("create TimeWheel tickMs:{}, wheelSize:{}, currentTime:{}", tickMs, wheelSize, this.currentTime);
    }

    public boolean add(TimerTaskEntry entry) {
        long expiration = entry.getExpireMs();

        log.info("current:{}, expire:{}, tickMs:{}, interval:{}", currentTime, expiration, tickMs, interval);

        if (expiration < tickMs + currentTime) {
            //到期了
            return false;
        } else if (expiration < currentTime + interval) {
            //扔进当前时间轮的某个槽里,只有时间大于某个槽,才会放进·123去
            long virtualId = (expiration -currentTime) / tickMs;
            int index = (int) (virtualId % wheelSize);

            log.info("virtualId,{}, index:{}", virtualId, index);
